...
const { BailErrorStrategy, DefaultErrorStrategy, FailedPredicateException } =
  antlr4.error;
...
export default class CustomErrorStrategy extends BailErrorStrategy {
  reportInputMismatch(
    ...
    e: antlr4.error.InputMismatchException,
  ) {
    let errorDisplay = DES.getTokenErrorDisplay(e.offendingToken);
    let expected = e
      .getExpectedTokens()
      .toString(
        recognizer.getLiteralNames() as string[],
        recognizer.getSymbolicNames() as string[],
      );
    expected = `{ ${expected} }`;
  ...
  reportNoViableAlternative(
    recognizer: antlr4.Parser,
    e: antlr4.error.NoViableAltException,
  ) {
    ...
    if (tokens !== undefined) {
      if (e.startToken.type === antlr4.Token.EOF) {
        ...
        const interval = new antlr4.Interval(
  ...
  reportFailedPredicate(
    recognizer: antlr4.Parser,
    e: antlr4.error.FailedPredicateException,
  ) {
